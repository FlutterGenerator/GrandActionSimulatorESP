namespace Offset {
    enum {
        ...
        _position = 0x341C610, // Transform -- get_position_injected
        _transform = 0x340AA04, // Component -- get_transform
        _main = 0x33BD668, // Camera -- get_main
        _worldtoscreenpoint = 0x33BCE4C, // Camera -- worldtoscreenpoint_injected
        _dead = 0x1227F28 // public bool Dead() { }
...
void *getTransform(void *player) {
    ...
    static const auto get_transform_injected = reinterpret_cast<uint64_t(__fastcall *)(
            void *)>(getAbsoluteAddress("libil2cpp.so", Offset::_transform));
...
Vector3 WorldToScreenPoint(void *transform, Vector3 test) {
    ...
    static const auto WorldToScreenPoint_Injected = reinterpret_cast<uint64_t(__fastcall *)(void *,
                                                                                            ...
                                                                                            int,
                                                                                            Vector3 &)>(getAbsoluteAddress(
            "libil2cpp.so", Offset::_worldtoscreenpoint));
...
bool GetPlayerDead(void *player) {
    if (!player) return NULL;
    static const auto get_Dead = reinterpret_cast<uint64_t(__fastcall *)(
            void *)>(getAbsoluteAddress("libil2cpp.so", Offset::_dead));
